...
class PayumProvider implements ServiceProviderInterface, ControllerProviderInterface
{
    ...
    {
        $app['payum.builder'] = function($app) {
        ...
        $app['payum.reply_to_symfony_response_converter'] = function($app) {
        ...
        $app['payum.gateway_factory_choices_callback'] = function ($app) {
        ...
        $app['payum.gateway_choices_callback'] = function ($app) {
            return function(Options $options) use ($app) {
                /** @var Payum $payum */
                $payum = $app['payum'];
        ...
        $app['payum.controller.notify'] = function() use ($app) {
            return new NotifyController($app['payum']);
        };
        $app['payum.controller.authorize'] = function() use ($app) {
            return new AuthorizeController($app['payum']);
        };

        $app['payum.controller.capture'] = function() use ($app) {
            return new CaptureController($app['payum']);
        };

        $app['payum.controller.refund'] = function() use ($app) {
